...
int main()
{
    ...
    while (true)
    {
        ...
        try
        {
            ...
            for (auto it = primes.rbegin(); it != primes.rend() && count < PRINT_MAX; ++it)
            {
            ...
            if (count >= PRINT_MAX)
            {
                std::cout << "\033[31mPrint Limit Reached: Last " << PRINT_MAX << " printed only\033[0m";
            }
        }
...
std::vector<int> return_prime_to(int n)
{
    ...
    if (n <= 0) throw std::invalid_argument("\033[31mreturn_prime_to: Must be a positive number.\033[0m");
